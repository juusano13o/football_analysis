...
class CameraMovementEstimator:
    def __init__(self, frame):
        ...
        self.lk_params = dict(
            winSize=(15, 15),
            maxLevel=2,  # down scale img upto twice
    ...
    def get_camera_movement(self, frames, read_from_stub=False, stub_path=None):
        ...
        # create camera movement per frame
        camera_movement = [[0, 0]] * len(frames)

        # convert img to gray img
        # so we can extract the feature
        # gray img from previous frames:
        ...
        for frame_num in range(1, len(frames)):
            ...
            # max distance between any 2 features
            max_distance = 0
            camera_movement_x, camera_movement_y = 0, 0

            for i, (new, old) in enumerate(zip(old_features, new_features)):
